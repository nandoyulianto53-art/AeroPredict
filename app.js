// Set real-time clock
        ...
        window.onload = function() {
            ...
            // AUTOMATIC LOCATION DETECTION ON LOAD (Menjawab request utama)
        ...
        async function detectUserLocation(manualClick = false) {
            ...
            // Atur waktu tunggu deteksi agar tidak berputar selamanya di beberapa HP lambat
            navigator.geolocation.getCurrentPosition(async (position) => {
                const lat = position.coords.latitude;
                const lon = position.coords.longitude;

                try {
                    // OpenStreetMap Nominatim API untuk menterjemahkan koordinat menjadi nama kota di Indonesia
                    const response = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}`);
                    const data = await response.json();
                    
                    const city = data.address.city || data.address.town || data.address.village || data.address.municipality || data.address.state || "Lokasi Anda";
                    
                    document.getElementById('input-wilayah').value = city;
                    
                    // Membuat data cuaca tiruan yang relevan berdasarkan lokasi
                    const mockSuhu = Math.round(28 + (Math.random() * 5));
                    const mockLembap = Math.round(65 + (Math.random() * 25));
                    const mockAngin = Math.round(6 + (Math.random() * 12));

                    document.getElementById('input-suhu').value = mockSuhu;
                    document.getElementById('input-kelembapan').value = mockLembap;
                    document.getElementById('input-angin').value = mockAngin;
                    updateSliders();

                    showToast(`📍 Lokasi terdeteksi: ${city}`, "success");

                    // Geser peta secara otomatis ke koordinat lokasi pengguna
                    if (mainMap) {
                        mainMap.setView([lat, lon], 12);
                        L.marker([lat, lon]).addTo(mainMap)
                            .bindPopup(`<b>Koordinat HP Anda</b><br>Kualitas udara dihitung berdasarkan sensor area ini.`)
                            .openPopup();
                    }
                    ...
                    // Jika API internet lambat, gunakan nama koordinat mentah
                    document.getElementById('input-wilayah').value = `GPS (${lat.toFixed(2)}, ${lon.toFixed(2)})`;
                    calculateAIPrediction();
                }
            }, (error) => {
                // Skenario jika GPS HP dinonaktifkan atau user menolak izin lokasi
                fallbackToDefault(manualClick ? "Izin lokasi ditolak atau GPS tidak aktif." : null);
            }, { enableHighAccuracy: true, timeout: 6000 });
        }

        // Penanganan fallback default jika GPS bermasalah
        ...
        function calculateAIPrediction() {
            ...
            // Persamaan tiruan pemodelan AI (PM2.5 dipicu oleh kelembapan tinggi, suhu tinggi, angin lambat)
            ...
            if (finalPM25 <= 50) {
                ...
                statusDesc = "Konsentrasi partikel PM2.5 berada pada tingkat ambang batas wajar. Kelompok sensitif (penderita asma) direkomendasikan berhati-hati.";
